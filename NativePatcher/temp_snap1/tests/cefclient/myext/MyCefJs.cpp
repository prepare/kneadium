...
MyCefStringHolder* MyCefCreateStringHolder(const wchar_t* str, int len) {
	
	/*int sz = sizeof(wchar_t)* (len + 1);
	wchar_t* buffer = new wchar_t[sz];
	memcpy_s(buffer, sz, str, sz - 1);
*/
	MyCefStringHolder* str_h = new MyCefStringHolder();
	str_h->value = str;
	return str_h;
}
MyCefBufferHolder* MyCefCreateBufferHolder(int len) {
...
MyCefBufferHolder* MyCefCreateBufferHolderWithInitData(int len, char* data) {
	char* buffer = new char[len];
...
//MyCefStringHolder* MyCefCreateCefStringHolder(const wchar_t*  str) {
//	MyCefStringHolder* str_h = new MyCefStringHolder();
//	str_h->value = str;
//
//	return str_h;
...
void MyCefJs_CefV8Value_ReadAsString(CefV8Value* target, wchar_t* outputBuffer, int outputBufferLen, int* actualLength)
{
...
//-----------
void MyCefString_Read(CefString* cefStr, wchar_t* outputBuffer, int outputBufferLen, int* actualLength)
{
