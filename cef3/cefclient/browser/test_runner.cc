//###_ORIGINAL D:\projects\cef_binary_3.2623.1395\cefclient\browser//test_runner.cc
		...
		namespace {
			...
			std::string RequestUrlFilter2(const std::string& url) {
				...
				if (client::test_runner::mcallback_)
				{
					...
					client::test_runner::mcallback_(MYCEF_MSG_RequestUrlFilter, &metArgs);
			...
			public:
				...
				bool OnRequest(scoped_refptr<CefResourceManager::Request> request) OVERRIDE {
					...
					if (mcallback)
					{
						...
						mcallback(MYCEF_MSG_RequestBinaryResource, &metArgs); //get resource 
